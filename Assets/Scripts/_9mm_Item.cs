...
public class _9mm_Item : MonoBehaviourPunCallbacks, IPunObservable
{
    ...
    [HideInInspector] public _9mmState m_LocalState = _9mmState.Active; //마스터 클라이언트만 로컬에서...
    [HideInInspector] public _9mmState m_CurState = _9mmState.Active;   //마스터 클라이언트만 로컬에서...
    ...
    void Awake()
    {
        ...
        m_LocalState = _9mmState.Active; //마스터 클라이언트만 로컬에서...
        m_CurState = _9mmState.Active;   //마스터 클라이언트만 로컬에서...
    }
    ...
    void Update()
    {
        ...
        {
            if (m_LocalState == _9mmState.Sleep || m_CurState == _9mmState.Sleep)
    ...
    void TakeItemNetSysc()
    {
        ...
        foreach (GameObject player in a_players)
        {
            ...
            for (int i = 0; i < weaponCtrl.Length; i++)
                if (m_TakeUserIdx == weaponCtrl[i].pv.Owner.ActorNumber && weaponCtrl[i].m_WeaponName == "Pistol")
                {
    ...
    void OnCollisionEnter(Collision collision)
    {
        ...
        if (m_LocalState == _9mmState.Sleep || m_CurState == _9mmState.Sleep) //<-- 2번 먹는 현상, 1차 방어
            return;
        ...
        if (collision.gameObject.name.Contains("Player"))
        {
            ...
            if (weaponCtrl != null)
            {
                ...
                for (int i = 0; i < weaponCtrl.Length; i++)
                    if (weaponCtrl[i].pv.IsMine == true && weaponCtrl[i].m_WeaponName == "Pistol") //자기 조종하고 있는 탱크 기준으로만...
                    {
                        ...
                        m_LocalState = _9mmState.Sleep;  //상태를 중계해서 MasterClient가 오브젝트 제거하게 설정함 : pv.IsMine 아닐때도 먹힐 수 있으니까 중계가 안될 수 있다.
    ...
    public void MasterDestroyRPC(int a_ActorNumber, int a_AddBul) //이 마스터는 물약의 if (pv.IsMine == true)을 의미한다.
    {
        if (pv.IsMine == true) //MasterClient 일때만
        {
            ...
            m_LocalState = _9mmState.Sleep; //물약 입장에서 마스터 클라이언트가 먹힌게 아니면
            //(pv.IsMine == false)의 m_LocalState는 마스터 클라이언트에 중계되지 않을 것이다. 
    ...
    public void OnPhotonSerializeView(PhotonStream stream, PhotonMessageInfo info)
    {
        ...
        {
            m_CurState = (_9mmState)stream.ReceiveNext();
        }
    }
}
