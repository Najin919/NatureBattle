...
public class GrenadeModelCtrl : MonoBehaviourPunCallbacks, IPunObservable
{
    ...
    [HideInInspector] public GrenadeModelState m_LocalState = GrenadeModelState.Active; //마스터 클라이언트만 로컬에서...
    [HideInInspector] public GrenadeModelState m_CurState = GrenadeModelState.Active;   //마스터 클라이언트만 로컬에서...
    ...
    void Awake()
    {
        ...
        m_LocalState = GrenadeModelState.Active; //마스터 클라이언트만 로컬에서...
        m_CurState = GrenadeModelState.Active;   //마스터 클라이언트만 로컬에서...
    }
    ...
    void Update()
    {
        ...
        {
            if (m_LocalState == GrenadeModelState.Sleep || m_CurState == GrenadeModelState.Sleep)
    ...
    void TakeItemNetSysc()
    {
        ...
        foreach (GameObject player in a_players)
        {
            ...
            if (m_TakeUserIdx == playerCtrl.pv.Owner.ActorNumber)
            {
                playerCtrl.pv.RPC("AddGrenadeCount", RpcTarget.AllViaServer);
                m_TakeOnece = false;
                break;
            }
        }
    ...
    void OnCollisionEnter(Collision collision)
    {
        ...
        if (m_LocalState == GrenadeModelState.Sleep || m_CurState == GrenadeModelState.Sleep) //<-- 2번 먹는 현상, 1차 방어
            return;
        ...
        if (collision.gameObject.name.Contains("Player"))
        {
            ...
            if (playerCtrl != null)
            {
                ...
                if (playerCtrl.pv.IsMine == true) //자기 조종하고 있는 탱크 기준으로만...
                {
                    m_TakeUserIdx = playerCtrl.pv.Owner.ActorNumber;

                    pv.RPC("MasterDestroyRPC", RpcTarget.MasterClient, m_TakeUserIdx); //MasterClient에게 RPC 보내서 이 오브젝트 제거 요청
                    m_LocalState = GrenadeModelState.Sleep;  //상태를 중계해서 MasterClient가 오브젝트 제거하게 설정함 : pv.IsMine 아닐때도 먹힐 수 있으니까 중계가 안될 수 있다.
    ...
    public void MasterDestroyRPC(int a_ActorNumber) //이 마스터는 수류탄의 if (pv.IsMine == true)을 의미한다.
    {
        if (pv.IsMine == true) //MasterClient 일때만
        {
            ...
            m_LocalState = GrenadeModelState.Sleep;
    ...
    public void OnPhotonSerializeView(PhotonStream stream, PhotonMessageInfo info)
    {
        ...
        {
            m_CurState = (GrenadeModelState)stream.ReceiveNext();
        }
    }
