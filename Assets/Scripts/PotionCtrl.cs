...
public class PotionCtrl : MonoBehaviourPunCallbacks, IPunObservable
{
    ...
    void Update()
    {
        ...
        {
            if (m_LocalState == PotionState.Sleep || m_CurState == PotionState.Sleep)
    ...
    void TakeItemNetSysc()
    {
        ...
        foreach (GameObject player in a_players)
        {
            ...
            if (m_TakeUserIdx == playerDamage.pv.Owner.ActorNumber)
            {
                playerDamage.pv.RPC("AddHealing", RpcTarget.AllViaServer, m_HealValue);
                m_TakeOnece = false;
                break;
            }
        }
    ...
    void OnCollisionEnter(Collision collision)
    {
        ...
        if (m_LocalState == PotionState.Sleep || m_CurState == PotionState.Sleep) //<-- 2번 먹는 현상, 1차 방어
            return;
        ...
        if (collision.gameObject.name.Contains("Player"))
        {
            ...
            if (playerDamage != null)
            {
                ...
                if (playerDamage.pv.IsMine == true) //자기 조종하고 있는 탱크 기준으로만...
                {
                    m_TakeUserIdx = playerDamage.pv.Owner.ActorNumber;
                    m_HealValue = 30.0f;

                    pv.RPC("MasterDestroyRPC", RpcTarget.MasterClient, m_TakeUserIdx, m_HealValue); //MasterClient에게 RPC 보내서 이 오브젝트 제거 요청
                    m_LocalState = PotionState.Sleep;  //상태를 중계해서 MasterClient가 오브젝트 제거하게 설정함 : pv.IsMine 아닐때도 먹힐 수 있으니까 중계가 안될 수 있다.
    ...
    [PunRPC]
    public void MasterDestroyRPC(int a_ActorNumber, float a_HVal) //이 마스터는 물약의 if (pv.IsMine == true)을 의미한다.
    {
        if (pv.IsMine == true) //MasterClient 일때만
        {
            ...
            m_LocalState = PotionState.Sleep; //물약 입장에서 마스터 클라이언트가 먹힌게 아니면
            //(pv.IsMine == false)의 m_LocalState는 마스터 클라이언트에 중계되지 않을 것이다. 
